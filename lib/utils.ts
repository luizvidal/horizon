...
export const authFormSchema = (type: "sign-in" | "sign-up") =>
	z.object({
		...
		state:
			type == "sign-in"
				? z.string().optional()
				: z.string().length(2, "State must be 2 characters long"),
		postalCode:
			type == "sign-in"
				? z.string().optional()
				: z
		...
		dateOfBirth:
			type == "sign-in"
				? z.string().optional()
				: z.string().length(10, "Date must be in this format: YYYY-MM-DD"),
		ssn:
			type == "sign-in"
				? z.string().optional()
				: z.string().min(3, "SSN must be at least 3 characters long"),
